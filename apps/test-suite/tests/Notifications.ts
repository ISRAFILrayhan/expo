    ...
    t.describe('scheduling', () => {
      ...
      t.describe('scheduleNotificationAsync', () => {
        ...
        onlyInteractiveDescribe('sound argument variations', () => {
          ...
          for (const soundValue of ['default', undefined, true, '__nonexistend_option']) {
            const soundLabel = String(soundValue);

            t.it(`triggers notification with sound: ${soundLabel} (default sound)`, async () => {
              await Notifications.scheduleNotificationAsync({
                content: {
                  title: `Sound: ${soundLabel}`,
                  body: 'Should play default sound',
                  sound: soundValue,
                },
                trigger: null,
              });
              await waitFor(1000);
              const soundWasCorrect = await askUserYesOrNo(
          ...
          t.it('triggers notification with sound: false (no sound)', async () => {
            await Notifications.scheduleNotificationAsync({
              content: {
                title: 'Sound: false',
                body: 'Should be SILENT (no sound)',
                sound: false,
              },
              trigger: null,
            });
            await waitFor(1000);
            const soundWasCorrect = await askUserYesOrNo('Was the notification SILENT (no sound)?');
            t.expect(soundWasCorrect).toBeTruthy();
          });

          t.it('triggers notification with custom sound file (pop_sound.wav)', async () => {
            await Notifications.scheduleNotificationAsync({
              content: {
                title: "Sound: 'pop_sound.wav'",
                body: 'Should play custom pop sound',
                sound: 'pop_sound.wav',
              },
              trigger: null,
            });
            await waitFor(1000);
            const soundWasCorrect = await askUserYesOrNo('Did you hear a custom "pop" sound?');
            t.expect(soundWasCorrect).toBeTruthy();
          });

          if (Platform.OS === 'ios') {
            t.it(
              "triggers notification with sound: 'defaultCritical' (iOS critical sound)",
              async () => {
                await Notifications.scheduleNotificationAsync({
                  content: {
                    title: "Sound: 'defaultCritical'",
                    body: 'Should play critical alert sound',
                ...
                await waitFor(1000);
                const soundWasCorrect = await askUserYesOrNo(
                  'Did you hear the critical alert sound (bypasses mute)?'
                );
                t.expect(soundWasCorrect).toBeTruthy();
              }
            );
            t.it("triggers notification with sound: 'defaultRingtone'", async () => {
              await Notifications.scheduleNotificationAsync({
                content: {
                  title: "Sound: 'defaultRingtone'",
                  body: 'Should play default ringtone',
