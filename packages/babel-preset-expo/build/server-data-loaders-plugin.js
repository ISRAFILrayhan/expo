...
function serverDataLoadersPlugin(api) {
    ...
    const collectReferencesAndNodes = (programPath) => {
        const refs = new Set();
        ...
        programPath.traverse({
            ...
            ExportDefaultDeclaration(exportPath) {
                if (t.isFunctionDeclaration(exportPath.node.declaration) &&
            ...
            Identifier(identifierPath) {
                ...
                if (binding && binding.path.type === 'FunctionDeclaration') {
                    const isSelfRef = identifierPath.findParent((p) => p === binding.path);
                    if (isSelfRef) {
                        counts.selfRefs++;
                    }
                }
    ...
    const sweepWithReferenceTracking = (programPath, state) => {
        let shouldContinue = true;
        let iterations = 0;
        while (shouldContinue) {
            shouldContinue = false;
            iterations++;
            const { refs, functionPaths, variablePaths, importPaths } = collectReferencesAndNodes(programPath);
            state.refs = refs;
            debug('Iteration', iterations, '- Reference set size:', state.refs.size);
            // Remove unreferenced functions
            for (const functionPath of functionPaths) {
                const identifier = getIdentifier(functionPath);
            ...
            for (const variablePath of variablePaths) {
                const identifier = getIdentifier(variablePath);
                if (identifier && !state.refs.has(identifier)) {
                    debug('Removing unreferenced variable:', identifier);
                    const parent = variablePath.parentPath;
                    variablePath.remove();
